...
import { mealsurvey } from '../../data/data';

const Meal = () => {
    const [step, setStep] = useState(0); // 단계
    const [responses, setResponses] = useState(Array(mealsurvey.length).fill(null)); // 답변 저장 배열
    ...
    return (
        <>
            ...
            <Container>
              <SurveyContent
                  step={step}
                  intro={mealsurvey[step].intro}
                  question={mealsurvey[step].question}
                  handleInputChange={handleInputChange}
                  isButtonEnabled={isButtonEnabled}
                  handleNextStep={handleNextStep}
                  currentInput={currentInput}
                  allowedCharsType={mealsurvey[step].allowedCharsType}
              />
            </Container>
        </>
    );
...
const SurveyContent = ({
    ...
    return (
      <>
        <ContainerTop>
          <StepText>{step + 1}/{mealsurvey.length}</StepText>
          <TextBox>
          ...
          <TextField 
            ...
            placeholder=""
            customText={mealsurvey[step].unit}
            width="321px"
          />
        </ContainerTop>
...
const AnswerButtons = ({ isButtonEnabled, handleNextStep }) => (
    <AnswerButtonContainer>
      <Button
        ...
        label={mealsurvey.length - 1 ? '완료' : '다음'}
        type="primary"
        size="medium"
