...
const Weight = () => {
    ...
    const handleNextStep = () => {
        ...
        localStorage.setItem('weight', JSON.stringify(newResponses));

        if (step < weight.length - 1) {
            setStep(step + 1);
    ...
    return (
        <>
            ...
            <Container>
              <SurveyContent
                  step={step}
                  intro={weight[step].intro}
                  question={weight[step].question}
                  handleInputChange={handleInputChange}
                  isButtonEnabled={isButtonEnabled}
                  handleNextStep={handleNextStep}
                  currentInput={currentInput}
                  allowedCharsType={weight[step].allowedCharsType}
              />
            </Container>
        </>
    );
...
const SurveyContent = ({
    ...
    return (
      <>
        <ContainerTop>
          <StepText>{step + 1}/{weight.length}</StepText>
          <TextBox>
